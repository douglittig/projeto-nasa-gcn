...
# MAGIC 3. Click on the **Edit** button next to the endpoint name.
# MAGIC
# MAGIC 4. **Add a New Served Entity**
# MAGIC     - In the **Served entities** section, click on **+ Add served entity**.
# MAGIC     - Select the entity name that matches your model name: **`genai_shared_catalog.ws_<xxx>.rag_app`**. Model name is printed in the beginning of this notebook.
...
# MAGIC     - Fill in the environment variables as follows (**💡These values are printed at the beginning of the lab. Replace the value of token_key and host_key**.):
...
# MAGIC     - **📌NOTE : Add a unique served entity name.**
# MAGIC
...
# MAGIC 9. **Rename the Served Entity**
# MAGIC     - In the Advanced Configuration section, update the Served entity name.
# MAGIC     - Change it from the default rag_app-1 to a unique name such as rag_app-2 for the **_new configuration_**.
# MAGIC         > If not updated, you may encounter the following error:
# MAGIC     `Served entities must have unique served entity names. Check the advanced configurations of your served entity.`
# MAGIC 10. **Update and Wait**
...
# MAGIC
# MAGIC In this task, you will query the model using Databricks SDK
