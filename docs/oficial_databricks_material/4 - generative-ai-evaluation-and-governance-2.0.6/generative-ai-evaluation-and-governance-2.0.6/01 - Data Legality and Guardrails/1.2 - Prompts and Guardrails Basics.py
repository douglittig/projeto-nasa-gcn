...
# MAGIC
# MAGIC Many LLMs, including Claude 3.7 Sonnet, have guardrails directly built into the solution:
# MAGIC
# MAGIC <br>
# MAGIC
# MAGIC ```
...
# MAGIC
# MAGIC To prevent the generation of toxic or unsafe content when using the Foundation Model API (FMAPI), enable the safety filter by setting `enable_safety_filter=True` within the `extra_body` parameter of your request. This configuration ensures that the model detects and filters out any unsafe content.
# MAGIC
# MAGIC <br>
# MAGIC
# MAGIC ```
